...
const s = (desc: string) => z.string().describe(desc);
const sOpt = (desc: string) => z.string().optional().describe(desc);

export const LessonPlanInputSchema = z.object({
  subject: s("The subject of the lesson plan."),
  topic: s("The specific topic for the week."),
  gradeLevel: s("The grade level of the students."),
  learningObjectives: s("The learning objectives for the week."),
  localLanguage: s("The local language for the lesson plan."),
  additionalDetails: sOpt(
    "Any additional details or context for the lesson plan."
  ),
});
...
const LessonPlanHistoryItemSchema = LessonPlanInputSchema.extend({
  weeklyPlan: z
    .string()
    .optional()
    .describe("The generated or edited lesson plan content."),
});
...
const GenerateLessonPlanOutputSchema = z.object({
  weeklyPlan: z
    .string()
    .describe("A detailed weekly lesson plan in Markdown format."),
});
...
const generateLessonPlanFlow = ai.defineFlow(
  ...
  async (input) => {
    ...
    if (!output) {
      throw new Error(
        "Failed to generate lesson plan from AI. The model did not return the expected output format."
      );
    }
    return output;
  }
